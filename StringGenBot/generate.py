...
async def generate_session(bot: Client, msg: Message, telethon=False, old_pyro: bool = False, is_bot: bool = False):
    ...
    if not is_bot:
        ...
        t = "يرجى إرسال  ** توكن البوت الخاص بك  ** للمتابعه.\مثال : `5432198765:abcdanonymousterabaaplol`'"
    phone_number_msg = await bot.ask(user_id, t, filters=filters.text)
    if await cancelled(phone_number_msg):
        return
    phone_number = phone_number_msg.text
    if not is_bot:
        await msg.reply("» محاولة إرسال OTP إلى nube المحدد...")
    else:
        await msg.reply("» تحاول تسجيل الدخول عبر رمز بوت ...")
    ...
    await bot.send_message(msg.chat.id, "» لقد تم الاستخراج بنجاح  {} جلسه البايروجرام.\n\nيرجى التححقق من قايمه الرسايل المحفوظه ! \n\n تم الاستخراج بواسطة سورس نجد @NGD_1 ♦".format("ᴛᴇʟᴇᴛʜᴏɴ" if telethon else "ᴩʏʀᴏɢʀᴀᴍ"))
